...
def prepare(out, working=False):
    ...
    tests = []
    for path in TEST_FILES + (["cmd/buildOutput_test.go", "cmd/generation_change_immutable_test.go"] if working else []):
...
def retain_evidence(out):
    import shutil
    destination = HERE / "native-command-results"
    destination.mkdir(exist_ok=True)
    for name in ["SOURCE.json", "expected-tests.json", "expected-mechanisms.json", "test-pattern.txt", "github-publication.json", "gce-resources.json", "gce-guest-attributes.json", "local-tests.jsonl", "local-mechanisms.jsonl", "local-serve-components.jsonl"]:
        path = out / name
        if path.exists():
